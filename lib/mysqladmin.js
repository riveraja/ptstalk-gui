    ...
    for (var key in keyList) {
        ...
        for (var v in stext) {
            if (stext[v][0] === keyList[key]) {
                var dp = [ stext[v][1], unixTimeStamp ];
                valuesArray.push(dp);
        ...
        for (var i in arrList) {
            ...
            if (row[0] === keyList[key]) {
                ...
                for (var step=1; step<row.length-1;step++) {
                    if (step === 1) {
                        var doc = [ parseInt(row[step]), unixTimeStamp ]
                        dpoints.push(doc);
                    } else {
                        unixTimeStamp = parseInt(unixTimeStamp) + parseInt(interVal);
                        var doc = [ parseInt(row[step+1] - row[step]), unixTimeStamp ]
                        dpoints.push(doc);
                    }
                }
                var outerDoc = {
                    "key": util.format("%s_delta", row[0]),
                    "datapoints": dpoints,
                    "type": "timeseries",
                    "startTime": unixTime,
